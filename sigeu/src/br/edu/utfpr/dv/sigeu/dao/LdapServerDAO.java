...
public class LdapServerDAO extends HibernateDAO<LdapServer> {
	...
	public List<LdapServer> pesquisa(String textoPesquisa, int limit) {
		if (textoPesquisa == null || textoPesquisa.trim().equals("")) {
			return this.pesquisa(limit);
		}
		String hql = "from LdapServer o where upper(o.sufixoEmail) like upper(:q) or upper(o.host) like upper(:q) order by o.ativo DESC, upper(o.sufixoEmail) ASC";
		Query q = session.createQuery(hql);
	...
	public List<LdapServer> pesquisa(int limit) {
		String hql = "from LdapServer o order by o.ativo DESC, upper(o.sufixoEmail) ASC";
		Query q = session.createQuery(hql);
		if (limit > 0) {
			q.setMaxResults(limit);
		}
